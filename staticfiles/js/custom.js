    ...
    function init_template() {
        ...
        function menuFunctions() {
            ...
            $('.menu-hider, .close-menu, .menu-close').on('click', function () {
                ...
                $('#footer-bar').removeClass('footer-menu-hidden');
                $('body').removeClass('modal-open');
                // changed modal aftermath
                $('.page-content').removeAttr('style');
                $('#footer-bar').attr('style', 'position:fixed');
                return false;
            });
        ...
        function darkMode() {
            ...
            $('[data-toggle-theme], [data-toggle-theme-switch]').off().on('click', function () {
                ...
                if ($('body').hasClass('theme-light')) {
                    ...
                    localStorage.setItem('Asterial-Theme', 'light-mode')
                }
                if ($('body').hasClass('theme-dark')) {
                    darkSwitch.prop('checked', true);
                    localStorage.setItem('Asterial-Theme', 'dark-mode')
                }
                setTimeout(function () { allowTransitions(); }, 150);
                return false;
            })
            if (localStorage.getItem('Asterial-Theme') == "dark-mode") { darkSwitch.prop('checked', true); $('body').removeClass('theme-light').addClass('theme-dark'); }
            if (localStorage.getItem('Asterial-Theme') == "light-mode") { darkSwitch.prop('checked', false); $('body').removeClass('theme-dark').addClass('theme-light'); }
        ...
        localStorage.setItem('z_Asterial-Visited:' + window.location.pathname, 'Page Visited');
        $.fn.visited = function (className) {
            className = className || false;
            $(this).each(function (index, element) {
                if (element.host == window.location.host &&
                    localStorage.getItem('z_Asterial-Visited:' + element.pathname) ||
                    localStorage.getItem('z_Asterial-Visited:' + element.pathname + '/')) {
                    if (className) {
        ...
        $('.list-group a').on('click', function () { if ($(this).attr('href') != '#') { localStorage.setItem('z_Asterial-Visited-Scroll-Position', $(window).scrollTop()); } })
        if ($('.check-visited').length) {
            setTimeout(function () {
                var getScroll = localStorage.getItem('z_Asterial-Visited-Scroll-Position');
                $(window).scrollTop(getScroll)
                localStorage.removeItem('z_Asterial-Visited-Scroll-Position');
            }, 200);
        }
        ...
        if (visitDetection.length) {
            var lastVisitValue = localStorage.getItem('Asterial-Last-Visited');
            var d = new Date();
            ...
            if (!lastVisitValue) {
                $('.never-visited').show();
                $('.before-visited').hide();
                localStorage.setItem('Asterial-Last-Visited', '<br>Your last visit was ' + strDate + ' at ' + strTime)
            } else {
        ...
        setTimeout(function () {
            ...
            $('.single-slider').owlCarousel({ loop: true, margin: 20, nav: false, lazyLoad: true, stagePadding: 30, items: 1, autoplay: true, autoplayTimeout: 4000 });
            $('.boxed-slider').owlCarousel({ loop: false, margin: 20, stagePadding: 50, nav: false, lazyLoad: true, items: 1, autoplay: true, autoplayTimeout: 4000 });
            ...
            $('.story-slider').owlCarousel({ loop: true, margin: 80, nav: false, stagePadding: 40, lazyLoad: true, items: 3, autoplay: false, dots: false, autoplayTimeout: 4000 });
            $('.cover-slider').owlCarousel({ loop: false, margin: 0, nav: false, lazyLoad: true, items: 1, autoplay: true, autoplayTimeout: 6000 });
            $('.double-slider').owlCarousel({ loop: true, stagePadding: 12, margin: 50, nav: false, items: 3, autoplay: true, autoPlayTimeout: 4000, dots: false });
            ...
            // custom sliders
            $('.main-banner-slider').owlCarousel({ loop: true, stagePadding: 0, margin: 5, nav: false, autoWidth: true, touchDrag: true, items: 1, autoplay: false, autoPlayTimeout: 4000, dots: false });
            $('.main-banner-slider-two').owlCarousel({ loop: true, margin: 10, nav: false, lazyLoad: true, autoWidth: true, touchDrag: true, stagePadding: 10, items: 1, autoplay: false, autoplayTimeout: 4000 });

            // custom slider for instructions screen
            $('.instruction-slider').owlCarousel({ loop: false, margin: 0, nav: false, lazyLoad: true, items: 1, autoplay: false, autoplayTimeout: 6000 });

            // custom slider for splash screen
            $('.splash-screen-slider').owlCarousel({ loop: false, margin: 0, nav: false, lazyLoad: true, items: 1, autoplay: false, autoplayTimeout: 6000 });
            // custom slider for product description page
            $('.ingredients-slider').owlCarousel({ loop: true, margin: 10, nav: false, lazyLoad: true, autoWidth: true, stagePadding: 10, items: 1, autoplay: false, autoplayTimeout: 4000 });
        }, 1000);
        setTimeout(function () {
        ...
        function highlightLoader() {
            $('[data-highlight-style]').on('click', function () {
                ...
                localStorage.setItem('Asterial-Highlight', getHighlight);
                $('#highlight').remove();
                $('head').append($('<link rel="stylesheet" id="highlight" type="text/css" />').attr('href', getHighlight));
                return false;
            });
            var rememberHighlight = localStorage.getItem('Asterial-Highlight');
